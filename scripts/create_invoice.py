...
def get_next_invoice_number(apartment, invoice_code):
    ...
    if not existing:
        return f"{invoice_code}_0001"
    
    # Extract numbers
    numbers = []
    for f in existing:
        match = re.search(rf'{invoice_code}_(\d+)', f.stem)
        if match:
            numbers.append(int(match.group(1)))
    
    next_num = max(numbers, default=0) + 1
    return f"{invoice_code}_{next_num:04d}"

def load_apartment_config(apartment, year):
    """Load apartment configuration."""
    config_path = PROJECT_ROOT / f"config/{apartment}_{year}.json"
...
def create_invoice(apartment, months, year, additional_emails=None):
    ...
    apartment_config = load_apartment_config(apartment, year)
    ...
    invoice_number = get_next_invoice_number(apartment, apartment_info['invoice_code'])
